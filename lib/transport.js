 ...
 */
async function execute({ text, destination, tags = [], addTimestamp = true, filenameFormat = '{timestamp} - {title}', category = '🟢' }) {
  // Validate inputs
  ...
  // Generate filename with format and category emoji
  const filename = generateFilename(title, filenameFormat, category);
 ...
 */
function generateFilename(title, format = '{timestamp} - {title}', category = '🟢') {
  const timestamp = generateTimestamp();
  const safeTitle = sanitizeForFilename(title) || 'Notiz';

  // Replace placeholders in format, insert category emoji before title
  let filename = format
    .replace('{timestamp}', timestamp)
    .replace('{title}', `${category} ${safeTitle}`);
